...
async def on_ready():
    ...
    await client.change_presence(status=discord.Status.idle,
                                 activity=discord.Game("mind tricks on you"))
...
async def uptime(ctx):
    ...
    await ctx.send(
        f"It has been {delta.day} days, {delta.hour} hours, {delta.minute} minutes and {delta.second} seconds since last accident.."
    )
    ...
    if interval == "secs" or "seconds":
        while 1:
            if datetime.datetime.now() >= start + datetime.timedelta(seconds=value):
                break
        await ctx.send(f"Done, {ctx.message.author.mention}")
    elif interval == "mins" or "minutes":
        while 1:
            if datetime.datetime.now() >= start + datetime.timedelta(minutes=value):
                break
        await ctx.send(f"Done, {ctx.message.author.mention}")
